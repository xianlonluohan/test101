...
// WS2812B RGB Pin Definition
constexpr gpio_num_t kWs2812bRgbPin = GPIO_NUM_4;
...
void InitMcpTools() {
  ...
  engine.AddMcpTool("self.ws2812b.set_index_color",      // tool name
                    "Set the color of a specific LED.",  // tool description
                    {
                        {
                            "index",
                            ai_vox::ParamSchema<int64_t>{
  ...
  engine.AddMcpTool("self.ws2812b.set_range_colors",    // tool name
                    "Set colors for a range of LEDs.",  // tool description
                    {
                        {
                            "start_index",
                            ai_vox::ParamSchema<int64_t>{
  ...
  engine.AddMcpTool("self.ws2812b.set_brightness",      // tool name
                    "Set the brightness of all LEDs.",  // tool description
                    {
                        {
                            "brightness",
                            ai_vox::ParamSchema<int64_t>{
  ...
  engine.AddMcpTool("self.ws2812b.get_index_color",      // tool name
                    "Get the color of a specific LED.",  // tool description
                    {
                        {
                            "index",
                            ai_vox::ParamSchema<int64_t>{
  ...
  engine.AddMcpTool("self.ws2812b.get_range_colors",       // tool name
                    "Get the colors of a range of LEDs.",  // tool description
                    {
                        {
                            "start_index",
                            ai_vox::ParamSchema<int64_t>{
  ...
  engine.AddMcpTool("self.ws2812b.get_brightness",  // tool name
                    "Get the current brightness.",  // tool description
                    {
                        // empty
                    }  // parameter schema
  );
...
std::string GetLedRangeColorsJson(const uint8_t start_index, const uint8_t end_index) {
  ...
  auto colors_array = cjson_util::ArrayMakeUnique();
  ...
  return cjson_util::ToString(colors_array, false);
}
...
void loop() {
  ...
  for (auto& event : events) {
    if (auto text_received_event = std::get_if<ai_vox::TextReceivedEvent>(&event)) {
      ...
      if ("self.audio_speaker.set_volume" == mcp_tool_call_event->name) {
      ...
      } else if ("self.ws2812b.set_index_color" == mcp_tool_call_event->name) {
        ...
        if (*index_ptr < 1 || *index_ptr > kWs2812bRgbLedNum) {
          engine.SendMcpCallError(mcp_tool_call_event->id, "Invalid index value, index must be between 1 and " + std::to_string(kWs2812bRgbLedNum));
          continue;
        }

        const auto red_ptr = mcp_tool_call_event->param<int64_t>("red");
        if (red_ptr == nullptr) {
          engine.SendMcpCallError(mcp_tool_call_event->id, "Missing required argument: red");
          continue;
        }
        if (*red_ptr < 0 || *red_ptr > 255) {
          engine.SendMcpCallError(mcp_tool_call_event->id, "Invalid red value, red must be between 0 and 255");
          continue;
        }

        const auto green_ptr = mcp_tool_call_event->param<int64_t>("green");
        if (green_ptr == nullptr) {
          engine.SendMcpCallError(mcp_tool_call_event->id, "Missing required argument: green");
          continue;
        }
        if (*green_ptr < 0 || *green_ptr > 255) {
          engine.SendMcpCallError(mcp_tool_call_event->id, "Invalid green value, green must be between 0 and 255");
          continue;
        }

        const auto blue_ptr = mcp_tool_call_event->param<int64_t>("blue");
        if (blue_ptr == nullptr) {
          engine.SendMcpCallError(mcp_tool_call_event->id, "Missing required argument: blue");
          continue;
        }
        if (*blue_ptr < 0 || *blue_ptr > 255) {
          engine.SendMcpCallError(mcp_tool_call_event->id, "Invalid blue value, blue must be between 0 and 255");
          continue;
        }

        printf("on mcp tool call: self.ws2812b.set_index_color, index: %" PRId64 ", red: %" PRId64 ", green: %" PRId64 ", blue: %" PRId64 "\n",
               *index_ptr,
               *red_ptr,
               *green_ptr,
               *blue_ptr);
        ...
        if (*start_index_ptr < 1 || *start_index_ptr > kWs2812bRgbLedNum) {
          engine.SendMcpCallError(mcp_tool_call_event->id,
                                  "Invalid start_index value, start_index must be between 1 and " + std::to_string(kWs2812bRgbLedNum));
          continue;
        }
        ...
        if (*end_index_ptr < 1 || *start_index_ptr > kWs2812bRgbLedNum) {
          engine.SendMcpCallError(mcp_tool_call_event->id,
                                  "Invalid end_index value, end_index must be between 1 and " + std::to_string(kWs2812bRgbLedNum));
          continue;
        }
        ...
        printf("on mcp tool call: self.ws2812b.set_range_colors, start_index: %" PRId64 ", end_index: %" PRId64 ", red: %" PRId64 ", green: %" PRId64
               ", blue: %" PRId64 "\n",
               *start_index_ptr,
               *end_index_ptr,
               *red_ptr,
        ...
        if (*brightness_ptr < 0 || *brightness_ptr > 255) {
          engine.SendMcpCallError(mcp_tool_call_event->id, "Invalid brightness value, brightness must be between 0 and 255");
          continue;
        }
        ...
        const std::string color_json = GetLedRangeColorsJson(static_cast<uint8_t>(*index_ptr), static_cast<uint8_t>(*index_ptr));
        printf("on mcp tool call: self.ws2812b.get_index_color, color: %s\n", color_json.c_str());
        engine.SendMcpCallResponse(mcp_tool_call_event->id, std::move(color_json));

      } else if ("self.ws2812b.get_range_colors" == mcp_tool_call_event->name) {
        const auto start_index_ptr = mcp_tool_call_event->param<int64_t>("start_index");
        if (start_index_ptr == nullptr) {
          engine.SendMcpCallError(mcp_tool_call_event->id, "Missing required argument: start_index");
          continue;
        ...
        if (*end_index_ptr < 1 || *end_index_ptr > kWs2812bRgbLedNum) {
          engine.SendMcpCallError(mcp_tool_call_event->id,
                                  "Invalid end_index value, end_index must be between 1 and " + std::to_string(kWs2812bRgbLedNum));
          continue;
        }
        ...
        const std::string color_json = GetLedRangeColorsJson(static_cast<uint8_t>(*start_index_ptr), static_cast<uint8_t>(*end_index_ptr));
        printf("on mcp tool call: self.ws2812b.get_range_colors, colors: %s\n", color_json.c_str());
        engine.SendMcpCallResponse(mcp_tool_call_event->id, std::move(color_json));
